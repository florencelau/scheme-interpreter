...
(define (zip pairs)
  (list (map (lambda (pair1) (car pair1)) pairs)
        (map (lambda (pair2) (cadr pair2)) pairs)
  )
)
...
(define (enumerate s)
  ; BEGIN Question 18
  (define (list-of-pairs index lst)
    (cond
      ((null? lst) nil)
      (else (cons (list index (car lst)) (list-of-pairs (+ index 1) (cdr lst))))
    )
  )
  (list-of-pairs 0 s)
)
