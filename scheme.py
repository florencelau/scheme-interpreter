...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Number of symbols does not match number of values")
        else:
...
class Procedure:
    ...
    def eval_call(self, arg_exprs, env, tail=False):
        ...
        if tail:
            args = arg_exprs.map(lambda operand: scheme_eval(operand, env, True))
        else:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try: 
            ...
            raise SchemeError("Wrong number of arguments passed in for function")
        # END Question 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 9A
        var = target.first
        env.define(var, LambdaProcedure(target.second, expressions.second, env))
        return var
        # END Question 9A
    else:
...
def do_quote_form(expressions, env):
    ...
    if expressions.second is nil: 
        ...
        return Pair(expressions.first, do_quote_form(expressions.second, env))
    # END Question 6B
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        return scheme_eval(expressions.first, env, True)
    elif scheme_false(scheme_eval(expressions.first, env)):
        return False
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        cond_predicate = scheme_eval(clause.first, env)
        if scheme_true(cond_predicate):
...
def make_let_frame(bindings, env):
    ...
    # BEGIN Question 16
    symbols, expressions = nil, nil
    while bindings is not nil:
        current = bindings.first
        check_form(current, 2, 2)
        if scheme_symbolp(current.first): 
            symbols = Pair(current.first, symbols)
            expressions = Pair(scheme_eval(current.second.first, env), expressions)
        else:
            raise SchemeError("Binding contains no symbol or incorrect syntax")
...
def check_formals(formals):
    ...
    if scheme_listp(formals):
        ...
        while formals is not nil:
            ...
            if formals.first in seen_symbols:
